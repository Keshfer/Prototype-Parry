    ...
    public override State RunCurrentState()
    {
        if (!coroutineRunning)
        {
            ...
            if (targetCollider != null)
            {
                ...
                Vector2 targetDirection = (targetCollider.transform.position - body.transform.position).normalized;
                RaycastHit2D hit = Physics2D.Raycast(body.transform.position, targetDirection, detectRadius, sightMask);
                int targetLayerNum = LayerMask.NameToLayer("Target");
                if (hit.collider != null && hit.collider.gameObject.layer.Equals(targetLayerNum))
                {

                    Debug.DrawRay(body.transform.position, targetDirection * detectRadius, Color.magenta);
